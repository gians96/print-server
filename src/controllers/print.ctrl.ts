...
const printCommand = async ({ body }: Request, res: Response) => {
    try {
        ...
        let response = await printCommandSrv(configPrinter, table)
        if (response.status) {
...
const printPreAccount = async ({ body }: Request, res: Response) => {
    try {
        const configPrinter: Printer = body.configPrinter
        const table: Table = body.table
        let response = await printPreAccountSrv(configPrinter, table)
        if (response.status) {
            res.status(200).send(response)
        } else {
            res.status(400).send(response)
...
const isConnectedPrint = async ({ body }: Request, res: Response) => {
    try {
        ...
        let response = await isConnectedPrintSrv(configPrinter)
        if (response.status) {
            res.status(200).send(response)
        } else {
            res.status(400).send(response)
